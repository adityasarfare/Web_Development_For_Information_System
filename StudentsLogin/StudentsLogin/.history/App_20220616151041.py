...
def api_update(id):
    ...
    if request.method == 'POST':
        ...
        cur.execute("UPDATE student SET first_name = ?, last_name = ?, email = ?, contact = ?, password = ?, gender = ?, hobbies = ?, country = ? WHERE id = ?",
                    (first_name,last_name,email,contact,password,gender,hobbies,country,id))

        connection.commit()

        result = cur.execute("SELECT * FROM students WHERE id = ?", (id))
        result = result.fetchall()
        col_name=["id","first_name","last_name","email","contact","password","gender","hobbies","country"]
        result_dict = []
