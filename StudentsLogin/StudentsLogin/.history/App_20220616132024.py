...
def api_create():  
    ...
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO students (id,first_name,last_name,email,contact,password,gender,hobbies,country) values (?,?,?,?,?,?,?,?,?)",
        (id,first_name,last_name,email,contact,password,gender,hobbies,country))
        connection.commit()

        result = cur.execute("SELECT * FROM students")
        result = result.fetchall()
        print(result)
        r = json.loads(str(json.dumps(result)).replace('\\', '').replace('"[', "[").replace("]\"", "]"))
        mydict = dict(ChainMap(*r))
