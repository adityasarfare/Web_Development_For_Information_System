...
def login():
    if request.method == 'POST':
        ...
        result = cur.execute("SELECT * FROM signup WHERE username = 'Barath'")
        result = cur.fetchall()
        usname=""
        passw = ""
        for i in result:
            if base64.b64decode(i[3]) == pwd:
                usname = i[1]
                passw = base64.b64decode(i[3])
                break
        if usname is not None | passw is not None:
