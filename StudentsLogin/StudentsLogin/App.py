...
def signup():
    ...
    if request.method == 'POST':
        ...
        result = cur.execute("SELECT * FROM signup WHERE email = ?", (email,))
        result = result.fetchall()
        if len(result) > 0:
            flash("User already exists, Try again with different mail")
            return render_template("signup.html")
        else:
...
def login():
    if request.method == 'POST':
        ...
        global is_Admin
        result = cur.execute("SELECT * FROM signup WHERE username = ?",(uname,))
        result = cur.fetchall()
        usname=""
        passw = ""
        for i in result:
            if base64.b64decode(i[3]) == pwd_enc:
                usname = i[1]
                passw = pwd
                break
        if usname and passw:
...
def admin():
    if request.method == 'POST':
        ...
        if request.form.get("username") == "admin" and request.form.get("password") == "admin":
           ...
           students = StudentModel.query.all()
           is_Admin = True
           response = make_response(render_template('adminpage.html', students = students, can_edit=is_Admin))
           response.headers['Content-Security-Policy'] = "style-src-elem' *.bootstrapcdn.com"
           return response
        #    return render_template('adminpage.html', students = students, can_edit=is_Admin)
        else:
            response1 = make_response(render_template('admin.html', failed = True))
            response1.headers['Content-Security-Policy'] = "style-src-elem' *.bootstrapcdn.com"
            return response1
